...
for pkg, vlist in versions_by_pkg.items():
    valid_dates = [v for v in vlist if v.published_at]
    if valid_dates:
        latest_v = max(valid_dates, key=lambda x: x.published_at)
...
for tgt_id in descendants_v:
    ...
    pkg_only, _ = tgt_id.split("@", 1)
    
    used_date = date_by_vid.get(tgt_id)
    latest_date = latest_date_by_pkg.get(pkg_only)
    
    print(f"Pkg: {tgt_id} | Used: {used_date} | Latest: {latest_date}")
